...
{
	setBitData(mDev->CR1, false, 6);	// SPI 비활성화
...
error Spi::send(void *src, int32_t  size)
{
	...
	mTxDma->lock();
#if defined(STM32F0)
	mDev->CR2 = SPI_CR2_TXDMAEN_Msk | 7 << 8 | 1 << 12;
#else
	...
	if(mDev->SR & SPI_SR_BSY_Msk)
	{
		mDev->DR;
#if defined(STM32F0)
		mDev->CR2 = SPI_CR2_RXNEIE_Msk | 7 << 8 | 1 << 12;
#else
...
#if defined(STM32F0)
	mDev->CR2 = SPI_CR2_TXDMAEN_Msk | SPI_CR2_RXDMAEN_Msk | 7 << 8 | 1 << 12;
#else
