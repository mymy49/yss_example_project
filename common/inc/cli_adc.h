...
#ifndef COMMON_ADC__H_
#define COMMON_ADC__H_
...
// ADC 측정을 하는 CLI를 명령을 생성한다.
// 사용 순서는 아래와 같다.
// 1. setNumOfAdc() 함수를 사용해 전체 LED 수를 설정한다.
// 2. setAdcChannel() 함수를 사용해 각 ADC 채널을 등록한다.
// 3. registerCli() 함수를 사용해 CommandLineInterface class의 instance에 등록한다.
namespace Cli
{
namespace Analog
{
	// 측정할 ADC의 전체 개수를 설정하는 함수이다.
	// 
	// uint8_t num
	//		ADC의 전체 개수를 설정한다.
	void setNumOfAdc(uint8_t num);

	// ADC의 측정 채널을 설정하는 함수이다.
	//
	// uint8_t num
	//		설정할 ADC의 인덱스를 설정한다.
	// uint8_t channel
